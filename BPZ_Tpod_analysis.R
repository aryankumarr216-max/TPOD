...
LCRD2_BPZ_Tpod_table <-LCRD.2(bmc_v_BPZ,probe_v_BPZ)
LCRD2_BPZ_Tpod <- LCRD2_BPZ_Tpod_table$BMC
...
First_mode_BPZ_table <- First.Mode(bmc_v_BPZ)
First_mode_BPZ_Tpod <- First_mode_BPZ_table$First_Mode
##################################################################
#tpod from 20th gene:
...
twenty_gene_BPZ_Tpod <- as.numeric(twenty_gene_BPZ_table[2])
##################################################################
...
tenth_percentile_BPZ_Tpod <- as.numeric(tenth_percentile_BPZ_table[2])
...
ggplot(df, aes(x = logBestBMD)) +
  ...
  geom_vline(xintercept = log10(LCRD2_BPZ_Tpod),
             linetype = "dashed",
             color = "blue") +
  
  geom_vline(xintercept = log10(First_mode_BPZ_Tpod),
             linetype = "dashed",
             color = "green") +
  
  geom_vline(xintercept = log10(twenty_gene_BPZ_Tpod),
             linetype = "dashed",
             color = "purple") +
  
  geom_vline(xintercept = log10(BPZ_Cat_tpod),
             linetype = "dashed",
             color = "pink") +
...
tpod_df <- data.frame(
  ...
  label = c(
    ...
    paste0("LCRD = ", round((LCRD2_BPZ_Tpod), 3)),
    paste0("First Mode = ", round((First_mode_BPZ_Tpod), 3)),
    paste0("20th Gene = ", round((twenty_gene_BPZ_Tpod), 3)),
    paste0("Category = ", round((BPZ_Cat_tpod), 3))
  )
)
